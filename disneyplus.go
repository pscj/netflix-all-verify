...
func RequestIP(requrl string, ip string, method string, proxyUrl string) string {
	var proxy *url.URL
...
func VerifyAuthorized(proxyUrl string) int {
	ipv4, _ := lookupIP("global.edge.bamgrid.com")

	tokenStatusv4 := RequestIP("https://global.edge.bamgrid.com/token", ipv4, "auth", proxyUrl)
	if tokenStatusv4 == "ok" {
		return 1
...
func QueryAreaAvailable(protocol string, proxyUrl string) string {
	ipv4, ipv6 := lookupIP("www.disneyplus.com")
	switch protocol {
	case "ipv4":
		return FindCountry(RequestIP("https://www.disneyplus.com", ipv4, "query", proxyUrl))
	case "ipv6":
		return FindCountry(RequestIP("https://www.disneyplus.com", ipv6, "query", proxyUrl))
	}
	return ""
}
